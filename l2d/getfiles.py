...
from utils.lua import readlua

import l2d.features as features
...
class DALSP_L2D:
    ...
    def copy(self, spirit_id, folder_name):
        ...
        # Edit model3 files + stage MLVE file
        for folder in os.listdir(curPath):
            ...
            self.get_bg_bgm()
            fileout = features.edit_model3(self.kanban_folder, self.model3_file,
    ...
    def getfile(self):
        ...
        if self.region.upper() != "CN":
            luastringPath = os.path.join(self.dataPath, r"src/lua/table/secondary/"+self.region.lower())
            self.string = readlua(os.path.join(luastringPath, stringfile))
        else:
            self.string = readlua(os.path.join(self.luatablePath, stringfile))
        self.role = readlua(os.path.join(self.luatablePath, rolefile))
        self.hero = readlua(os.path.join(self.luatablePath, herofile))
        self.dress = readlua(os.path.join(self.luatablePath, dressfile))
        ...
        if self.spirit_need == "all":
            ...
            # generate "All" MLVE
            setattr(self.options, "all", True)
            mlve_gen = DALSP_L2D_mlve(self.options)
            mlve_gen.genmlve()
        else:
